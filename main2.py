...
# You can find the list of supported regions in the Vertex AI documentation.
REGION = "us-central1"
# ---------------------------------------------------------------
...
def generate_plan_from_xml(xml_string):
    ...
    try:
        model = GenerativeModel(MODEL_NAME)
        response = model.generate_content(prompt)
        json_plan_str = response.text.strip().replace("```json\n", "").replace("```", "")
        json_plan = json.loads(json_plan_str)
        return json_plan, prompt, None
    except Exception as e:
        return None, prompt, f"Error generating plan: {e}"
...
def generate_sql_from_plan(plan):
    ...
    try:
        ...
        response = model.generate_content(prompt)
        sql_query = response.text.strip().replace("```sql\n", "").replace("```", "")
        return sql_query, prompt, None
    except Exception as e:
        return None, prompt, f"Error generating SQL from plan: {e}"
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen p-4">
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-4xl">
        ...
        <div class="mt-8">
            ...
            <div class="mb-6">
                <h3 class="text-lg font-medium text-gray-700 mb-2">Prompt for Planner Agent (Step 1)</h3>
                <div class="bg-gray-800 text-gray-200 rounded-md p-4 overflow-x-auto shadow-inner">
                    <pre><code class="whitespace-pre-wrap">{{ plan_prompt }}</code></pre>
                </div>
            </div>
            <div class="mb-6">
            ...
            <div class="mb-6">
                <h3 class="text-lg font-medium text-gray-700 mb-2">Prompt for Generator Agent (Step 2)</h3>
                <div class="bg-gray-800 text-gray-200 rounded-md p-4 overflow-x-auto shadow-inner">
                    <pre><code class="whitespace-pre-wrap">{{ sql_prompt }}</code></pre>
                </div>
            </div>
            <div>
...
def generate():
    ...
    if use_a2a:
        ...
        json_plan, plan_prompt, error = generate_plan_from_xml(xml_input)
        if error:
            return render_template_string(HTML_TEMPLATE, error_message=error, plan_prompt=plan_prompt)

        sql_query, sql_prompt, error = generate_sql_from_plan(json_plan)
        if error:
            return render_template_string(HTML_TEMPLATE, error_message=error, json_plan=json_plan, plan_prompt=plan_prompt, sql_prompt=sql_prompt)
            
        return render_template_string(HTML_TEMPLATE, sql_query_a2a=sql_query, json_plan=json_plan, plan_prompt=plan_prompt, sql_prompt=sql_prompt)
    else:
